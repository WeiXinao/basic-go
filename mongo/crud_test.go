...
func TestMongo(t *testing.T) {
	...
	opts := options.Client().ApplyURI("mongodb://root:123456@192.168.5.4:27017/").
		SetMonitor(monitor)
	...
	updateRes, err := col.UpdateMany(ctx, filter, bson.D{{"$set", bson.D{{"title", "新的标题"}}}})
	assert.NoError(t, err)
	fmt.Println("affected", updateRes.ModifiedCount)
	updateRes, err = col.UpdateMany(ctx, filter, bson.D{bson.E{Key: "$set", Value: Article{Title: "我的标题2", AuthorId: 123456}}})
	assert.NoError(t, err)
	fmt.Println("affected", updateRes.ModifiedCount)
